...
def azure_blobcontainer_create(task_id: str, params: dict) -> ACTaskSpec:
    name = params.get("name", f"accontainer{task_id}")
    ...
    verify = VerifyPlan(
        ...
        steps=[{"op": "blobContainers.get", "name": name, "storageAccount": storage_account}],
    )
    def verify_fn(spec, validator_ctx):
        ...
        try:
            ...
            container = client.blob_containers.get(resource_group, storage_account, name)
            return container.name == spec.params["name"]
        except Exception:
            return False
    spec = ACTaskSpec(
        task_id=task_id,
        provider="azure",
        kind="azure.blobcontainer.create",
